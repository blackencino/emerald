...
void Triangle::init() {
    ...
    // Barycentric Matrix.
    m_barycentricMatrix = M33f(m_p2[0].x,
                               m_p2[0].y,
                               1.0f,
                               m_p2[1].x,
                               m_p2[1].y,
    ...
    m_center = (m_vertices[0]->position() + m_vertices[1]->position() +
...
static bool _testPointVsSegmentNoSwap(const V2f& A,
                                      const V2f& B,
                                      const V2f& P) {
    bool say = A.y >= P.y;
    bool sby = B.y >= P.y;
...
static inline bool testPointVsSegment(const V2f& a,
    ...
    if (b.y < a.y || (b.y == a.y && b.x < a.x)) {
        return _testPointVsSegmentNoSwap(b, a, p);
    } else {
        return _testPointVsSegmentNoSwap(a, b, p);
    }
}
...
bool Triangle::hit2D(const V2f& p, float d) const {
    ...
    if (m_bounds2D.intersects(p) == false) { return false; }
...
bool Triangle::intersectSegment(const V3f& origin,
    ...
    if (!sBnds.intersects(m_bounds)) { return false; }
    ...
    V2f p2;
    switch (m_projectionAxis) {
    case 0: p2 = V2f(p.y, p.z); break;
    case 1: p2 = V2f(p.x, p.z); break;
    case 2: p2 = V2f(p.x, p.y); break;
    };
    ...
    if (NC < 2) { NC += (int)testPointVsSegment(m_p2[0], m_p2[2], p2); }
...
V3f Triangle::closestPoint(const V3f& p, float& retD2) const {
    ...
    V2f p2;
    switch (m_projectionAxis) {
    case 0: p2 = V2f(projP.y, projP.z); break;
    case 1: p2 = V2f(projP.x, projP.z); break;
    case 2: p2 = V2f(projP.x, projP.y); break;
    };
    ...
    if (NC == 1) {
        ...
        for (int edge = 0; edge < 3; ++edge) {
            ...
            if (edgeProj >= 0.0f && edgeProj <= m_edgeLengths[edge]) {
                V3f edgePoint =
                  m_edgeOrigins[edge] + edgeProj * m_edgeUnitDirs[edge];
...
void Triangle::computeBarycentric(const V3f& i_p,
    ...
    V3f planePos = m_vertices[0]->position() + (dp - (o_elevation * m_planeN));

    // Now project into 2D.
    V3f p2;
    switch (m_projectionAxis) {
    case 0: p2 = V3f(planePos.y, planePos.z, 1.0f); break;
    case 1: p2 = V3f(planePos.x, planePos.z, 1.0f); break;
    case 2: p2 = V3f(planePos.x, planePos.y, 1.0f); break;
    };
